...
resource "null_resource" "update_permissions" {
  provisioner "local-exec" {
    command = <<EOT
      echo '${tls_private_key.mykey.private_key_openssh}' > ./insureme-prod-ec2.pem
      chmod 400 ./insureme-prod-ec2.pem
    EOT
  }
  triggers = {
...
resource "aws_ssm_parameter" "ec2_private_key" {
  name        = "/capstone/project3/insureme-prod-ec2_private_key"
...
resource "aws_ssm_parameter" "ec2_public_key" {
  name        = "/capstone/project1/insureme-prod-ec2_public_key"
